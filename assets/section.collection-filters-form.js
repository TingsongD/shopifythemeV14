(function($){
  ...
  function getUsableCollectionFiltersForm(context) {
    const scopedForm = context && context.closest ? context.closest('collection-filters-form') : null;
    const form = scopedForm || document.querySelector('collection-filters-form');

    return form && typeof form.onActiveFilterClick === 'function' ? form : null;
  }
  ...
  function getFallbackSortUrl(sortValue) {
  ...
  class CollectionFiltersForm extends HTMLElement {
    constructor() {
      ...
      if(form) form.addEventListener('input', this.debouncedOnSubmit.bind(this));
    ...
    startRender() {
      ...
      if(collectionProductGrid && collectionProductGrid.parentNode && theme.Preloader) {
        theme.Preloader.set(collectionProductGrid.parentNode, {
          fixed: true,
          spinner: theme.current.is_mobile ? false : null
        });
      }

      if(theme.current.is_mobile && collectionSidebar && theme.Preloader) {
        theme.Preloader.set(collectionSidebar, {
            fixed: true
        });
      }
    }
    ...
    endRender() {
      ...
      if(collectionProductGrid && collectionProductGrid.parentNode && theme.Preloader) {
        theme.Preloader.unset(collectionProductGrid.parentNode);
      }

      if(theme.current.is_mobile && collectionSidebar && theme.Preloader) {
        theme.Preloader.unset(collectionSidebar);
      }
    ...
    renderPage(searchParams, event, updateURLHash = true, params = {}) {
      ...
      if(!collectionProductGrid) {
        navigateToHref(event && event.currentTarget ? event.currentTarget.href : null);
        return;
      }
      ...
      collectionProductGrid.classList.add('loading');
      ...
      if(!sections.length || !theme.collection || !theme.collection.enable_ajax || event && event.reloadCollection || (Shopify.designMode && !params.appendContent)) {
        if(!url) {
          url = `${pathname ? pathname : window.location.pathname}${searchParams ? '?' : ''}${searchParams}`;
        }
      ...
      fetch(url)
        ...
        .catch(error => {
          ...
          if (window.ThemeErrorHandler) {
            ThemeErrorHandler.handleFetchError(error, 'Collection filters fetch');
          }
        });
    }
    ...
    renderProductGrid(html, event, params) {
      ...
      // Null check - exit early if elements not found
      if (!collectionProductGrid || !parsedHTMLCollectionProductGrid) {
        if(window.ThemeDebug) ThemeDebug.warn('CollectionProductGrid element not found');
        return;
      }
      ...
      if(params.appendContent) {
        // Safe DOM insertion - doesn't re-parse existing content
        const fragment = document.createRange().createContextualFragment(
          parsedHTMLCollectionProductGrid.innerHTML
        );
        collectionProductGrid.appendChild(fragment);
      } else {
      ...
      if(params.scrollToTop) {
        ...
        // Native smooth scroll - no jQuery animation needed
        const targetY = collectionProductGrid.getBoundingClientRect().top
          + window.pageYOffset
          - header_h;
      ...
      collectionProductGrid.classList.remove('loading');
    }
    ...
    renderProductCount(html) {
      ...
      // Only update if both source and target exist
      if (container && count) {
        container.innerHTML = count.innerHTML;
        container.classList.remove('loading');
      }
      if (containerDesktop && count) {
        containerDesktop.innerHTML = count.innerHTML;
        containerDesktop.classList.remove('loading');
      }
      if (containerMobileFilterButton) {
        const mobileCount = parsedHTML.getElementById('CollectionProductCountMobileFilterButton');
        if (mobileCount) {
          containerMobileFilterButton.innerHTML = mobileCount.innerHTML;
        }
      }
    }
    ...
    renderFilters(html, event) {
      ...
      if(event) {
        if(event.isCollectionChanged || event.state && event.state.isCollectionChanged) {
          ...
          if(collectionFiltersForm) {
            const targetForm = document.querySelector('#CollectionFiltersForm');
            if (targetForm) {
              if(theme.RangeOfPrice) theme.RangeOfPrice.destroy();
              targetForm.innerHTML = collectionFiltersForm.innerHTML;
      ...
      if(processFacets) {
        ...
        Array.from(facetDetailsElementsFromDom).forEach((currentElement) => {
          const sidebarSection = currentElement.closest('.js-collection-sidebar-section');
          
          if(!sidebarSection) return;
          
          if (!Array.from(facetDetailsElementsFromFetch).some(({ id }) => currentElement.id === id)) {
            sidebarSection.classList.add('d-none');
          } else {
            sidebarSection.classList.remove('d-none');
          }
        });

        const matchesIndex = (element) => { 
          const eventTarget = event && event.target && event.target.closest ? event.target : null;
          const jsFilter = eventTarget && !event.isHistoryChange ? eventTarget.closest('.js-filter') : undefined;
        ...
        facetsToRender.forEach((elementToRender, index) => {
          if (!elementToRender || !elementToRender.id) return;

          const currentElement = document.getElementById(elementToRender.id);
          // Element already rendered in the DOM so just update the innerHTML
          if (currentElement && elementToRender.innerHTML !== undefined) {
            ...
            if (index > 0) {
              ...
              if (elementToRender.className === previousElementClassName && previousElement) {
                previousElement.after(elementToRender);
                return;
              }
            }

            if (elementToRender.parentElement && elementToRender.parentElement.id) {
              const parentFilter = document.querySelector(`#${elementToRender.parentElement.id} .js-filter`);
              if (parentFilter) {
                parentFilter.before(elementToRender);
              }
            }
          }
        });

        if(countsToRender && event && event.target && event.target.closest) this.renderCounts(countsToRender, event.target.closest('.js-filter'));
    ...
    renderShFilters(html) {
      ...
      if(shFilter.length) {
        shFilter.forEach(element => {
          ...
          if (parsedHTMLShFilter && parsedHTMLShFilter.innerHTML !== undefined) {
    ...
    renderNavigation(html) {
      ...
      elements.forEach((element, index) => {
        if(!parsedHTMLLinksElements[index]) return;
    ...
    renderPagination(html) {
      const parsedHTML = new DOMParser().parseFromString(html, 'text/html');
      const paginationTarget = document.getElementById('CollectionPagination');
      const paginationSource = parsedHTML.getElementById('CollectionPagination');

      if (paginationTarget && paginationSource) {
        paginationTarget.innerHTML = paginationSource.innerHTML;
      }
    }
    ...
    renderBreadcrumbs(html) {
      ...
      const dynamicBreadcrumbs = parsedHTML.getElementById('DynamicCollectionBreadcrumbs');

      if (collectionBreadcrumbs && dynamicBreadcrumbs) {
        collectionBreadcrumbs.innerHTML = dynamicBreadcrumbs.innerHTML;
      }
    }
    ...
    getSections() {
      const section = document.getElementById('main-collection-product-grid');

      if(!section || !section.dataset.id) return [];
  ...
  class FacetRemove extends HTMLElement {
    constructor() {
      ...
      link.addEventListener('click', (event) => {
        const form = getUsableCollectionFiltersForm(this);
  ...
  class CollectionNavigationForm extends HTMLElement {
    ...
    onSubmitHandler(event) {
      ...
      const form = getUsableCollectionFiltersForm();
      const sourceForm = form ? form.querySelector('form') : null;

      if(!form || !sourceForm || typeof form.onHistoryChange !== 'function') {
        navigateToHref(url);
        return;
      }
  ...
  class CollectionSortingCloneForm extends HTMLElement {
    ...
    onSubmitHandler(event) {
      ...
      const formData = new FormData(event.target.closest('form'));
      const sortValue = formData.get('sort_by_clone');
      const form = getUsableCollectionFiltersForm();
      const sourceForm = form ? form.querySelector('form') : null;
      const sortInput = form ? form.querySelector('[name="sort_by"]') : null;

      if(!form || !sourceForm || !sortInput) {
        window.location.href = getFallbackSortUrl(sortValue);
        return;
      }

      sortInput.value = sortValue;
      sourceForm.dispatchEvent(new Event('input'));
    }
  }
  ...
  class CollectionPaginationItem extends HTMLElement {
    constructor() {
      ...
      link.addEventListener('click', this.goToPage.bind(this));
      link.addEventListener('loadmore', this.goToPage.bind(this));
    }
    ...
    goToPage(event) {
      ...
      if(!form) {
        if(event.type !== 'click') navigateToHref(event.currentTarget && event.currentTarget.href);
        return;
      }
